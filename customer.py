...
class Cust_Win:
    def __init__(self, root):
        ...
        # =======================logo=============
        img2 = Image.open(r"C:\Users\Dell\Desktop\hotel-management\image\logohotel.png")
        ...
        labelframeleft = LabelFrame(self.root, bd=2, relief=RIDGE, text="Customer Details",
                                    font=("time new roman", 12, "bold"), padx=2)
        ...
        enty_ref = ttk.Entry(labelframeleft, width=29, textvariable=self.var_ref, font=("time new roman", 13, "bold"),
                             state="readonly")
        ...
        enty_ref = ttk.Entry(labelframeleft, textvariable=self.var_cust_name, width=29, font=("arial", 13, "bold"))
        ...
        enty_ref = ttk.Entry(labelframeleft, textvariable=self.var_mother, width=29, font=("arial", 13, "bold"))
        ...
        enty_ref = ttk.Entry(labelframeleft, textvariable=self.var_post, width=29, font=("arial", 13, "bold"))
        ...
        enty_ref = ttk.Entry(labelframeleft, textvariable=self.var_mobile, width=29, font=("arial", 13, "bold"))
        ...
        enty_ref = ttk.Entry(labelframeleft, textvariable=self.var_email, width=29, font=("arial", 13, "bold"))
        ...
        combo_Nationality = ttk.Combobox(labelframeleft, textvariable=self.var_nationality,
                                         font=("time new roman", 12, "bold"), width=27, state="readonly")
        ...
        # id number
        lblIdNumber = Label(labelframeleft, textvariable=self.var_id_number, text="Id Number", padx=2, pady=6)
        ...
        enty_ref = ttk.Entry(labelframeleft, textvariable=self.var_address, width=29, font=("arial", 13, "bold"))
        ...
        btnAdd = Button(btn_frame, text="Add", command=self.add_data, font=("time new roman", 12, "bold"), bg="black",
                        fg="gold", width=8)
        btnAdd.grid(row=0, column=0, padx=1)

        btnUpadate = Button(btn_frame, text="Update",command=self.update, font=("time new roman", 12, "bold"), bg="black", fg="gold",
                            width=10)
        btnUpadate.grid(row=0, column=1, padx=1)

        btnDelete = Button(btn_frame, text="Delete", command=self.mDelete,font=("time new roman", 12, "bold"), bg="black", fg="gold",
                           width=10)
        btnDelete.grid(row=0, column=2, padx=1)

        btnReset = Button(btn_frame, text="Reset", command=self.reset,font=("time new roman", 12, "bold"), bg="black", fg="gold", width=10)
        ...
        btnSearch = Button(Tabel_Frame, text="Search",command=self.search, font=("arial", 12, "bold"), bg="black", fg="gold", width=8)
        btnSearch.grid(row=0, column=3, padx=1)

        btnShowAll = Button(Tabel_Frame, text="Show All",command=self.fetch_data, font=("arial", 12, "bold"), bg="black", fg="gold",
                            width=8)
        btnShowAll.grid(row=0, column=4, padx=1)

        # ==============show data tabel================
        deatils_table = Frame(Tabel_Frame, bd=2, relief=RIDGE)
        deatils_table.place(x=0, y=50, width=860, height=350)

        scroll_x = ttk.Scrollbar(deatils_table, orient=HORIZONTAL)
        scroll_y = ttk.Scrollbar(deatils_table, orient=VERTICAL)

        self.Cust_Details_Table = ttk.Treeview(deatils_table, columns=(
        "ref", "name", "mother", "gender", "post", "mobile", "email", "nationality", "idproof", "idnumber", "address"),
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(host="localhost", username="root", password="12345", database="management")
        my_cursor = conn.cursor()
    ...
    def search(self):
        ...
        # y_cursor.execute("select * from customer where"+str(self.search_var.get)+" LIKE '%"+str(self.txt_search.get())+"%'")
        rows=my_cursor.fetchall()
        if len (rows)!=0:
            self.Cust_Details_Table.delete(*self.Cust_Details_Table.get_children())
            for i in rows:
